...
export function validateContent(data) {
  ...
  if (!view || typeof view !== "object" || Array.isArray(view)) {
    E("product_view 必须是对象，每期给出一个最高优先级产品判断");
  } else {
    if (!DECISION_AREAS.has(view.area)) E(`product_view.area 应为：${[...DECISION_AREAS].join(" / ")}`);
    for (const [field, min, max] of [
      ...
      if (typeof value !== "string" || value.trim().length < min || value.trim().length > max) {
        E(`product_view.${field} 应为 ${min}–${max} 字符`);
      }
    }
    if (!["高", "中", "低"].includes(view.confidence)) E("product_view.confidence 只能是 高 / 中 / 低");
  }
