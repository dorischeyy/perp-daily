...
// 主来源承载条目 date；补充来源明确标出背景事实或交叉验证，不挤进正文。
const renderSources = (it) => {
  const links = [];
  const primary = safeUrl(it.url);
  if (primary) links.push(`<a class="item-src" href="${esc(primary)}" target="_blank" rel="noopener">${esc(it.source || "来源")} ↗</a>`);
  else if (it.source) links.push(`<span class="item-src">${esc(it.source)}</span>`);
  for (const ref of Array.isArray(it.references) ? it.references : []) {
    const url = safeUrl(ref?.url);
    const date = typeof ref?.date === "string" && /^\d{4}-\d{2}-\d{2}$/.test(ref.date) ? ` · ${ref.date.slice(5)}` : "";
    if (url) links.push(`<a class="item-src item-ref" href="${esc(url)}" target="_blank" rel="noopener">补充：${esc(ref.source || "来源")}${esc(date)} ↗</a>`);
  }
  return links.join("");
};

// 「今日进展」：只渲染当天有可核验新 delta、且没有正文承载的故事线。
// 每条 = {title, since, day_n, tier, status, date, source, url, update, watch}。
const renderThreads = (threads) => {
  ...
  const row = (t) => {
    ...
    const source = t.source
      ? (url
          ? `<a class="thread-src" href="${esc(url)}" target="_blank" rel="noopener">${esc(t.source)} ↗</a>`
          : `<span class="thread-src">${esc(t.source)}</span>`)
      : "";
    return `
        <div class="thread">
          ...
          ${(t.date || source) ? `<div class="thread-source">${t.date ? esc(t.date) : ""}${t.date && source ? " · " : ""}${source}</div>` : ""}
          ${t.watch ? `<div class="thread-w"><b>下一步看：</b>${esc(t.watch)}</div>` : ""}
        </div>`;
  };
  return `
    <section class="threads">
      <div class="threads-h">今日进展</div>
      ${threads.map(row).join("")}
    </section>`;
};

// 顶部产品判断：把整期信息压缩成一个可争论、可证伪的结论。
const renderProductView = (view) => {
  if (!view || typeof view !== "object") return "";
  return `
    <section class="product-view">
      <div class="product-view-area">${esc(view.area || "")} · 置信度 ${esc(view.confidence || "")}</div>
      <div class="product-view-j">${mdInline(view.judgment || "")}</div>
      <div class="product-view-row"><b>判断变化：</b>${mdInline(view.change || "")}</div>
      <div class="product-view-row product-view-f"><b>证伪条件：</b>${mdInline(view.falsifier || "")}</div>
    </section>`;
};

// 栏目顺序：Perp DEX 第一，再 Launchpad、Crypto、AI，最后「产品判断」收尾。
...
// 产品判断栏(hertzflow)是研判综述、可引用较老来源，不渲染日期，避免页面显示老日期被误读为旧闻。
const renderItem = (it, hideDate = false) => `
        <article class="item">
          ${it.decision_area ? `<div class="decision-area">${esc(it.decision_area)}</div>` : ""}
...
<style>
  ...
  *{box-sizing:border-box; margin:0; padding:0;}
  ...
  /* 最高优先级产品判断 */
  .product-view{margin:22px 0 8px; padding:20px 22px; background:#17150f; color:#fbfaf6;
    border-radius:7px; overflow-wrap:anywhere;}
  .product-view-area{font-family:-apple-system,"PingFang SC",sans-serif; font-size:11px;
    color:#a79f8d; margin-bottom:8px;}
  .product-view-j{font-size:19px; font-weight:700; line-height:1.55; margin-bottom:12px;}
  .product-view-row{font-size:13.5px; line-height:1.7; color:#d9d2c2;}
  .product-view-row b{color:#fbfaf6;}
  .product-view-f{margin-top:4px; color:#b8ae99;}
  /* 今日进展：只展示有来源的新 delta */
  ...
  .thread-source{font-family:-apple-system,"PingFang SC",sans-serif; font-size:11px;
    color:#a3987f; margin-top:4px;}
  .thread-src{color:#8a7f66; text-decoration:none;}
  .thread-src:hover{color:#b03a2e;}
  ...
  .decision-area{display:inline-block; margin-bottom:5px; padding:1px 7px; border-radius:10px;
    background:#efe5ca; color:#8a6c19; font-family:-apple-system,"PingFang SC",sans-serif;
...
<body>
  <main class="mag">
    ...
    ${renderProductView(data.product_view)}
