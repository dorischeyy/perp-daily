...
- 同一条进展只选择一个主要承载位置：有完整新闻条目就用“正文 + 一句内联回扣”；没有独立条目但到期状态值得说，才用顶部「持续追踪」。不得同时在 lead、持续追踪和正文复述事件事实。
...
**重写约束**：
- **lead = 今日唯一最重要的判断**：一句话只给 house view，不重复第一条新闻的主体、时间、金额和事件细节，别把三件不相干的事塞进一个长句。
...
**发布前重复性检查**：逐层对照 `lead → 持续追踪 → 栏目标题 → item headline/body → context → 机会与打法`。同一事件事实、数字或结论若出现两次，明确一个主要承载位置，其余改成连接关系、判断或直接删除。`lib/check-editorial.mjs` 会阻断完全重复、lead/首段近似复述、context 复述正文，以及格式空泛或未由当期新闻触发的机会项。
