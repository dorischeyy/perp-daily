...
这是全报唯一集中写产品决策的地方，通常 1-3 条。栏目标题固定为「机会与打法」，不显示「对 HertzFlow 的启发」，也不再加重复 kicker。
...
每条必须按三步写：
1. **判断**：明确写“现在做 / 进入候选 / 先验证 / 保持不动 / 不做”中的一种，不用空泛的“建议评估”。
2. **机制对照**：点出现有机制已经覆盖什么、真正缺口是什么，不能把现有能力写成建议。
3. **动作与验收**：给一个具体动作，以及进入下一步所需的数据、阈值、压力测试结果或外部事件。

挂触发判断的来源 URL，不点名具体职位或角色。若不能形成完整的“新闻事实 → 现有机制 → 差异 → 动作 → 验收”，就省略该条；没有合格条目就整栏省略。
...
```json
{
  ...
  "sections": [
    ...
    // 可选，仅当有真动作时追加；title 固定，且不设置 kicker：
...
> **机会与打法格式（硬规则）**：每条严格三段 `**判断** / **机制对照** / **动作与验收**`，最多 3 条，URL 必须对应当期新闻；不设置 `context`，避免把判断再摘要一遍。
