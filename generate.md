...
**发布前重复性检查**：逐层对照 `lead → 持续追踪 → 栏目标题 → item headline/body → context → 机会与打法`。同一事件事实、数字或结论若出现两次，明确一个主要承载位置，其余改成连接关系、判断或直接删除。
...
```json
{
  ...
  "sections": [
    { "id": "perpdex", "title": "Perp DEX",
      "items": [ { "headline": "标题", "body": ["**事件**：发生了什么(精简,带 benchmark 判读)", "**机制**：怎么回事", "**二阶效应**：……"], "source": "来源名", "url": "https://...", "date": "YYYY-MM-DD", "context": { "label": "量级参照", "text": "一句同口径的量级解释" } } ] },
