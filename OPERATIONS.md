...
7. 不再生成或展示顶部 `product_view`；导语后直接进入「今日进展」或新闻栏目。
8. 「产品判断」为可选末栏，最多 2 条；严格使用判断变化、证据与边界、对 HertzFlow 三段，且必须由当期新闻触发。
9. lead 只用一句话概括近期市场情况，必须由当期最重要的 2-3 个新闻主体、动作与阶段归纳而来，不得写脱离新闻要点的抽象口号。
...
- 页面不含 `product_view`、判断变化、置信度或证伪条件卡片，导语后直接进入新闻内容
